...
from model.utils import evaluation, mAP
...
def de_diag(acc, each_angle=False):
    ...
    # np.diag(np.diag(acc)) returns the matrix that have a similar shape with x but only contains the diagonal elements
    # here np.diag(np.diag(acc)) shape = (11, 11)
    result = np.sum(acc - np.diag(np.diag(acc)), 1) / 10.0
    if not each_angle:
        result = np.mean(result)
    return result

with open("work/data.pickle", "rb") as f:
    data = pickle.load(f)

print('Evaluating...')
acc, num_rank = evaluation(data, conf['data'])
...
for i in range(1):
    print('===Rank-%d (Include identical-view cases)===' % (num_rank))
...
for i in range(1):
    print('===Rank-%d (Exclude identical-view cases)===' % (num_rank))
    print('NM: %.3f,\tBG: %.3f,\tCL: %.3f' % (
        de_diag(acc[0, :, :, i]),
        de_diag(acc[1, :, :, i]),
        de_diag(acc[2, :, :, i])))
...
for i in range(1):
    print('===Rank-%d of each angle (Exclude identical-view cases)===' % (num_rank))
